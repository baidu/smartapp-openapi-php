...
class GetAnalysisTerminalInfoRequest {
    public $accessToken; // string 授权小程序的接口调用凭据
    public $terminalType; // string 终端数据类型
...
class GetAnalysisTerminalInfo{
    ...
    public function doRequest($params){
        ...
        $client->setPath("/rest/2.0/smartapp/data/getanalysisterminal");
        $client->setContentType("application/x-www-form-urlencoded");

        $client->addGetParam("sp_sdk_lang", SDKLANG, true);
        $client->addGetParam("sp_sdk_ver", SDKVERSION, true);
        $client->addGetParam("access_token", $params->accessToken, true);
        $client->addPostParam("terminal_type",  $params->terminalType, true);
